...
int		ft_ldpwr(char *s)
{
	...
	{
		if (s[49] == '1')
			pwr = (s[i] == '1') ? pwr + ft_atoi(ft_pwr2(63 - i)) : pwr;
		else
			pwr = (s[i] == '0') ? pwr + ft_atoi(ft_pwr2(63 - i)) : pwr;
	}
	pwr = (s[49] == '1') ? pwr : 1 - pwr;
	return (pwr);
}
...
char	*ft_realpart(char *s, int pwr, int znak)
{
	...
	num2 = "0";
	k = pwr > 63 ? pwr - 63 : 0;
	ft_ldarrnew(s, 0, pwr + 1, r);
	s = ft_strrev(r);
	while (s[++i])
		num2 = ft_strrev_free(ft_strsum(ft_strrev_free(num2, 0),
		ft_strrev_free((s[i] == '1') ? ft_pwr2(i + k) : "0", 0)), 1);
...
char *ft_ldseparat(char *s, int pwr, int c)
{
	...
	else
	{
		...
		mant = s;
		while (++i < -pwr - 1)
			mant = ft_strjoin_free("0", mant, 0, 0);
	}
...
char	*ft_ld(long double *d, t_ld *list)
{
	...
	ft_bzero(ld, 129);
	ft_bzero(list->ld, 129);
	ft_ldbit(ld, d, sizeof(*d));
	ft_ldbit(list->ld, d, sizeof(*d));
	ft_ldarrnew(ld, 64, 128, mant);
	ft_ldarrnew(list->ld, 64, 128, list->mant2);
	return (ft_ldseparat(mant, ft_ldpwr(ld), ld[48]));
}
